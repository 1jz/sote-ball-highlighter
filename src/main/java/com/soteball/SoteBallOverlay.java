...
class SoteBallOverlay extends Overlay {
    private final Client client;
    private final SoteBallPlugin plugin;
    private final SoteBallConfig config;

    @Inject
    private SoteBallOverlay(Client client, SoteBallPlugin plugin, SoteBallConfig config) {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    public Dimension render(Graphics2D graphics) {
        client.getWorldView(-1).getProjectiles().forEach(p -> {
            int id = p.getId();
            boolean outlineProjectile = false;
            Color color = Color.BLACK;

            if (SoteBallConstants.RANGE_IDS.contains(id)) {
                color = config.defaultRangeColour();
                outlineProjectile = true;
            } else if (SoteBallConstants.MAGE_IDS.contains(id)) {
                color = config.defaultMageColour();
                outlineProjectile = true;
            } else if (SoteBallConstants.DEATH_BALL_IDS.contains(id)) {
                color = config.defaultDeathBallColour();
                outlineProjectile = true;
            }

            if (outlineProjectile) {
    ...
    // ================================================================
    // Credits to contributors of ModelOutlineRenderer.java in Runelite
    // ================================================================

    private static class IntBlockBuffer {
